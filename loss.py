...
class LossComputer:
    def __init__(
        ...
        self.group_counts = torch.Tensor(dataset.group_counts).to("cuda")
        self.group_frac = self.group_counts / self.group_counts.sum()
        self.group_str = dataset.group_str

        if adj is not None:
            self.adj = torch.from_numpy(adj).float().to("cuda")
        else:
            self.adj = torch.zeros(self.n_groups).float().to("cuda")

        if is_robust:
            assert alpha, "alpha must be specified"

        # quantities maintained throughout training
        self.adv_probs = torch.ones(self.n_groups).to("cuda") / self.n_groups
        self.exp_avg_loss = torch.zeros(self.n_groups).to("cuda")
        self.exp_avg_initialized = torch.zeros(self.n_groups).byte().to("cuda")
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        group_map = (
            group_idx == torch.arange(self.n_groups).unsqueeze(1).long().to("cuda")
        ).float()
    ...
    def reset_stats(self):
        self.processed_data_counts = torch.zeros(self.n_groups).to("cuda")
        self.update_data_counts = torch.zeros(self.n_groups).to("cuda")
        self.update_batch_counts = torch.zeros(self.n_groups).to("cuda")
        self.avg_group_loss = torch.zeros(self.n_groups).to("cuda")
        self.avg_group_acc = torch.zeros(self.n_groups).to("cuda")
