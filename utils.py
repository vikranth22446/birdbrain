...
class CSVBatchLogger:
    def __init__(self, csv_path, n_groups, mode="w", wandb=False, logger_mode="train"):
        columns = ["epoch", "batch"]
    ...
    def log(self, epoch, batch, stats_dict):
        ...
        updated_stats_dict = {
            f"{self.logger_mode}/{k}": v for k, v in stats_dict.items()
        }
        if self.wandb:
            wandb.log(updated_stats_dict)
